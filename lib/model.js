...
function RModel(definitions) {
    ...
    var fileds = util.map(key.match(r), function(filed) {
        return filed.slice(1, -1);
    });
...
RModel.prototype.generateKey = function(params) {
    ...
    if (length === 1 && util.isObject(params) === false) {
        ...
        if (util.isArray(params) === false) {
            _params = params;
            params = [];
            util.each(_params, function(filed) {
                p = params[filed];
                if (p) params.push(p);
            });
        }

        if (params.length !== fileds.length) {
            throw new Error('the params do not match all the fileds: ' + fileds.toString() + '. params: ' + JSON.stringify(params));
        }
