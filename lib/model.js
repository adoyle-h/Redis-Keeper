...
RModel.prototype.generateKey = function(params) {
    ...
    if (length === 1) {
        if (checkParamType(params) === false) {
            throw new Error('the param should be a Number or String.');
        }
        ...
        if (util.isArray(params) === false) {
            if (util.isObject(params) === false) {
                throw new Error('params should be an Array or Object.');
            }

            _params = params;
            params = [];
            util.each(_params, function(p) {
                if (checkParamType(p)) params.push(p);
            });
        }

        if (params.length !== fields.length) {
            var missing = util.difference(fields, params);
            throw new Error('the params do not match all the fields: ' + fields.toString() + '. Actual missing: ' + missing.toString());
        }
